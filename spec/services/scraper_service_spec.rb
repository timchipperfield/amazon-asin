...
RSpec.describe ScraperService, type: :model do
  ...
  describe "#scrape" do
    ...
    context "when a real asin is passed to the service" do
      context "when asin matches html layout #1", vcr: {cassette_name: "banana_toothbrush_request"} do
        let(:asin) { "B002QYW8LW" }
      ...
      context "when asin matches html layout #3", vcr: {cassette_name: "refurbished_laptop_request"} do
        let(:asin) { "B06XD3LXXK" }
